...
import com.tci.service.EmployeeServiceImpl;
...
public class TciAssignmentApplicationTests {
    ...
    public void testSaveEmployees_DepartmentNotFound() {
        ...
        employeeDTOs.add(new EmployeeDTO("Ankit Singh", "IT", (double) 5000, "USD", LocalDate.now(), LocalDate.now().plusYears(1)));

        // Invoke the service method
        employeeServiceImpl.saveEmployees(employeeDTOs);
    ...
    public void testSaveEmployees_DepartmentExists() {
        ...
        // Prepare test data
        List<EmployeeDTO> employeeDTOs = new ArrayList<>();
        employeeDTOs.add(new EmployeeDTO("Ankit Singh", "IT", (double) 6000, "INR", LocalDate.now(), LocalDate.now().plusYears(1)));
    ...
    public void testSaveEmployees_NullValuesInDTO() {
        ...
        List<EmployeeDTO> employeeDTOs = new ArrayList<>();
        employeeDTOs.add(new EmployeeDTO(null, "IT", (double) 5000, "USD", LocalDate.now(), LocalDate.now().plusYears(1)));
    ...
    public void testGetEligibleEmployees() {
        ...
        employees.add(new Employee(1L, "Ankit Singh", (double) 5000, "USD", LocalDate.parse("2022-01-01"), LocalDate.parse("2023-01-01"), new Department(1L, "IT")));
        employees.add(new Employee(2L, "Vishal Singh", (double) 6000, "INR", LocalDate.parse("2021-01-01"), LocalDate.parse("2022-01-01"), new Department(2L, "Accounts")));
        when(employeeRepository.findAll()).thenReturn(employees);

        // Invoke the service method
        LocalDate eligibilityDate = LocalDate.parse("2022-06-01");
        List<Employee> eligibleEmployees = employeeServiceImpl.getEligibleEmployees(eligibilityDate);

        // Verify interactions
        verify(employeeRepository, times(1)).findAll();

        // Assertions
        assertAll("eligibleEmployees",
                () -> assertEquals(1, eligibleEmployees.size()), // Assuming only one employee is eligible
